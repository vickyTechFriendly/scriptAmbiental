...
async function getHumedad() {
  return fetchData("2579");
}

async function getPresion() {
    return fetchData("2580");
}
...
async function getRuido() {
    return fetchData("2586");
}

async function getNO() {
    return fetchData("2573");
}

async function getCO() {
    return fetchData("2574");
}

async function getO3() {
    return fetchData("2575");
}

async function getSO2() {
    return fetchData("2576");
}

async function getNO2() {
    return fetchData("2577");
}

async function getTemperatura() {
    return fetchData("2578");
  }

async function getPM25() {
    return fetchData("2581");
}

async function getVV() {
    return fetchData("2582");
}

async function getDD() {
    return fetchData("2583");
}

async function getPM10() {
    return fetchData("2585");
}

async function getICA() {
    return fetchData("2778");
}

const plataforma = "smart.albacete.es";
const accessToken = process.env.estacion3;
  ...
  async function sendTelemetry() {
    try { 
        ...
        if(Object.keys(telemetryData).length === 0) {
            console.log("No hay datos diponibles para la estación 3.");
            return;
        }
        ...
        await axios.post(`https://${plataforma}/api/v1/${accessToken}/telemetry`, telemetryData,{
            ...
            console.log("Datos enviados estación 3:", telemetryData);
