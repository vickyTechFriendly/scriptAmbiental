...
async function getHumedad() {
  return fetchData("2593");
}

async function getPresion() {
    return fetchData("2594");
}

async function getParticulasPM1() {
    return fetchData("2598");
}

async function getRuido() {
    return fetchData("2600");
}

async function getNO() {
    return fetchData("2587");
}

async function getCO() {
    return fetchData("2588");
}

async function getO3() {
    return fetchData("2589");
}

async function getSO2() {
    return fetchData("2590");
}

async function getNO2() {
    return fetchData("2591");
}

async function getTemperatura() {
    return fetchData("2592");
  }

async function getPM25() {
    return fetchData("2595");
}

async function getVV() {
    return fetchData("2596");
}

async function getDD() {
    return fetchData("2597");
}

async function getPM10() {
    return fetchData("2599");
}

async function getICA() {
    return fetchData("2779");
}

const plataforma = "smart.albacete.es";
const accessToken = process.env.estacion4;
let HR;
let TMP;
let PRB;
let PM1;
  ...
  async function sendTelemetry() {
    try { 
        ...
        if (dataHumedad && dataHumedad.length > 0) {
            ...
            console.log("No hay datos de HR disponibles para la estacion 4.");
            return; 
        }
        if (dataTemperatura && dataTemperatura.length > 0) {
            TMP = dataTemperatura[0].valor;
        } else {
            console.log("No hay datos de temperatura disponibles para la estacion 4.");
            return; 
        }
        if (dataPresion && dataPresion.length > 0) {
            PRB = dataPresion[0].valor;
        } else {
            console.log("No hay datos de presión disponibles para la estacion 4.");
            return; 
        }
        if (dataParticulasPM1 && dataParticulasPM1.length > 0) {
            PM1 = dataParticulasPM1[0].valor;
        } else {
            console.log("No hay datos de particulas PM1 disponibles para la estacion 4.");
            return; 
        }
        if (dataRuido && dataRuido.length > 0) {
            R = dataRuido[0].valor;
        } else {
            console.log("No hay datos de ruido disponibles para la estacion 4.");
            return; 
        }
        if (dataNO && dataNO.length > 0) {
            NO = dataNO[0].valor;
        }else {
            console.log("No hay datos de NO disponibles para la estacion 4.");
            return; 
        }
        if (dataCO && dataCO.length > 0) {
            CO = dataCO[0].valor;
        }else {
            console.log("No hay datos de CO disponibles para la estacion 4.");
            return;
        }
        if (dataO3 && dataO3.length > 0) {
            O3 = dataO3[0].valor;
        }else {
            console.log("No hay datos de O3 disponibles para la estacion 4.");
            return;
        }
        if (dataNO2 && dataNO2.length > 0) {
            NO2 = dataNO2[0].valor;
        }else {
            console.log("No hay datos de NO2 disponibles para la estacion 4.");
            return;
        }
        if (dataPM25 && dataPM25.length > 0) {
            PM25 = dataPM25[0].valor;
        }else {
            console.log("No hay datos de PM25 disponibles para la estacion 4.");
            return;
        }
        if (dataVV && dataVV.length > 0) {
            VV = dataVV[0].valor;
        }else {
            console.log("No hay datos de VV disponibles para la estacion 4.");
            return;
        }
        if (dataDD && dataDD.length > 0) {
            DD = dataDD[0].valor;
        }else {
            console.log("No hay datos de DD disponibles para la estacion 4.");
            return;
        }
        if (dataPM10 && dataPM10.length > 0) {
            PM10 = dataPM10[0].valor;
        }else {
            console.log("No hay datos de PM10 disponibles para la estacion 4.");
            return;
        }
        if (dataSO2 && dataSO2.length > 0) {
            SO2 = dataSO2[0].valor;
        }else {
            console.log("No hay datos de SO2 disponibles para la estacion 4.");
            return;
        }
        if (dataICA && dataICA.length > 0) {
            ICA = dataICA[0].valor;
        }else {
            console.log("No hay datos de ICA disponibles para la estacion 4.");
            return;
        }
        ...
        console.log("Datos estación 4: ", telemetryData);
